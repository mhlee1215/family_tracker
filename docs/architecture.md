...
Camping saved searches are account-scoped server data stored under the authenticated user's family/user boundary. The browser keeps a local copy only as a fallback and syncs saved campground queries through `/api/camping/queries`. Campground search is unified through `/api/camping/search`, with provider adapters for Recreation.gov national campgrounds and ReserveCalifornia state park campgrounds. Recurring availability checks are centrally owned by the server-side camping monitor, configured through `/api/camping/monitor` and executed through `/api/camping/monitor/run` or the Node scheduler tick. The browser only saves queries, updates monitor settings, asks for immediate runs, and renders the saved results returned by the server. Browser code must not own long-lived camping polling loops because closing the page would stop them and because global pause/concurrency/load-balancing controls belong at the server boundary.
