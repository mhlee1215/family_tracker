...
export class SQLiteBabyStore {
  ...
  getNotificationSettings(options = {}) {
    ...
    `).get(familyId, babyId, userId);
    return row ? rowToNotificationSettings(row) : defaultNotificationSettings({ familyId, babyId, userId });
  }

  saveNotificationSettings(settings, options = {}) {
  ...
  savePushSubscription(subscription, options = {}) {
    ...
    return this.listPushSubscriptionsForUser({ familyId, userId }).find((item) => item.endpoint === subscription.endpoint) || null;
  }

  listPushSubscriptionsForUser(options = {}) {
    const familyId = options.familyId || defaultFamilyId;
    const userId = options.userId || '';
    return this.db.prepare(`
    ...
    `).all(familyId, userId).map(rowToPushSubscription);
  }

  disablePushSubscription(endpoint, options = {}) {
    const familyId = options.familyId || defaultFamilyId;
    const userId = options.userId || '';
    const now = new Date().toISOString();
    this.db.prepare(`
    ...
    `).run(now, now, familyId, userId, endpoint);
  }

  disablePushSubscriptionByEndpoint(endpoint) {
    const now = new Date().toISOString();
    this.db.prepare(`
      UPDATE push_subscriptions
      SET disabled_at = ?, updated_at = ?
  ...
  replacePendingNotificationJob(job, options = {}) {
    ...
    try {
      ...
      `).run(now, familyId, babyId, userId, type, job.dedupeKey);
      this.db.prepare(`
    ...
    return this.getNotificationJob(job.id) || this.getNotificationJobByDedupeKey(job.dedupeKey);
  }

  cancelPendingNotificationJobs(options = {}) {
    const familyId = options.familyId || defaultFamilyId;
    const babyId = options.babyId || defaultBabyId;
    const userId = options.userId || '';
    ...
    `).run(now, familyId, babyId, userId, type);
  }

  listDueNotificationJobs(options = {}) {
    ...
    return this.db.prepare(`
      ...
      LIMIT ?
    `).all(now, limit).map(rowToNotificationJob);
  }

  getNotificationJob(id) {
    const row = this.db.prepare('SELECT * FROM notification_jobs WHERE id = ?').get(id);
    return row ? rowToNotificationJob(row) : null;
  }

  getNotificationJobByDedupeKey(dedupeKey) {
    const row = this.db.prepare('SELECT * FROM notification_jobs WHERE dedupe_key = ?').get(dedupeKey);
    return row ? rowToNotificationJob(row) : null;
  }

  markNotificationJobSent(id, options = {}) {
  ...
  markNotificationJobFailed(id, options = {}) {
    const now = options.failedAt || new Date().toISOString();
    this.db.prepare(`
      UPDATE notification_jobs
      SET status = ?, failure_reason = ?, updated_at = ?
      WHERE id = ?
    `).run(options.status || 'failed', String(options.failureReason || '').slice(0, 500), now, id);
  }
